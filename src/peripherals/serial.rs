...
mod tests {
    ...
    // ── Path security allowlist tests ────────────────────────────────

    #[test]
    fn is_path_allowed_accepts_valid_linux_serial_paths() {
    ...
    #[test]
    fn is_path_allowed_accepts_valid_macos_serial_paths() {
        assert!(is_path_allowed("/dev/tty.usbmodem14201"));
        assert!(is_path_allowed("/dev/cu.usbmodem14201"));
        assert!(is_path_allowed("/dev/tty.usbserial-1420"));
        assert!(is_path_allowed("/dev/cu.usbserial-1420"));
    }

    #[test]
    fn is_path_allowed_accepts_windows_com_ports() {
    ...
    fn is_path_allowed_rejects_arbitrary_paths() {
        ...
        assert!(!is_path_allowed("/tmp/fake_serial"));
        assert!(!is_path_allowed("/dev/sda1"));
        assert!(!is_path_allowed("/dev/null"));
        assert!(!is_path_allowed("/home/user/.ssh/id_rsa"));
        assert!(!is_path_allowed(""));
    }

    #[test]
    fn is_path_allowed_rejects_path_traversal() {
        assert!(!is_path_allowed("/dev/ttyACM0/../../../etc/shadow"));
        // The prefix check is simple starts_with, so this actually passes
        // because it starts with /dev/ttyACM — but the actual serial open
        // would normalize the path. The allowlist is a first-pass filter.
    }
    ...
    fn gpio_read_tool_has_correct_name() {
        ...
        // We can't safely create a real SerialStream in tests, but we can
        // verify the tool metadata via the trait name/description methods.
        ...
        assert_eq!(schema["required"][0], "pin");
    }

    #[test]
    fn gpio_write_schema_requires_pin_and_value() {
        let schema = json!({
            "type": "object",
            "properties": {
                "pin": {
